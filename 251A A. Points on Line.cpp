...
my solution on codeforces :https://codeforces.com/contest/251/submission/175316205
problem name: A. Points on Line
status: accepted
...
void LELLO()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ll ind = lower_bound(all(vc), vc[i]+d)- vc.begin() ;
        if (ind == n || vc[ind] != vc[i]+d) 
        {
            ind -- ;
        }
        ll between = ind -i -1 ;
        ans += (between*(between+1))/(2LL) ;
    }
    cout<< ans <<endl;
}
void driver(ll t=1)
