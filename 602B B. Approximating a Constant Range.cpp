...
my solution on codeforces : https://codeforces.com/contest/602/submission/175052957
...
void LELLO()
{
    ...
    int r =0 , l =0 ;
    while(r <n){
        // need to store the occurrences of elements 
        mp[vc[r]] ++;
        // store unique element in segment 
        st.insert(vc[r]);
        while((*st.rbegin()-*st.begin()) > 1){
            mp[vc[l]] --;
            // get rid of the m in my segment
            if(!mp[vc[l]]){
                st.erase(vc[l]) ;
            }
            l++;
