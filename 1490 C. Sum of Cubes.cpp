...
my solution on codeforces : https://codeforces.com/contest/1490/submission/174510343
problem name: C. Sum of Cubes
status: accepted
...
void LELLO()
{
    ...
    // }
    for (ll i = 1 ; i <= rem ; i++){
        double long b = cbrt(n - pow(i , 3)) ;
        double long  fra = modf(b , &ff ) ;
        if(fra == 0.0 && pow(i, 3)+pow(b, 3) == n && b !=0){
            cout<<"YES"<<endl;
            bye
        }
    }
